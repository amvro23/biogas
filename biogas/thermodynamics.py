...
# ------------------------------------------------------------------------------------------------
# HEAT, ENTROPY, GIBBS OF FORMATION
# ------------------------------------------------------------------------------------------------    
...
def calc_sf_temp(T, sf_298, a, b, c, d):
    """
    Returns the entropy of formation in kJ/kmol.
    Parameters
    ----------
    T : float or int
        Temperature in K.
    sf_298 : 1d array, float or int
        Contains entropy of formation at 298.15K.
    a : 1d array, float or int
        Contains a coefficient of the heat capacity.
    b : 1d array, float or int
        Contains a coefficient of the heat capacity.
    ...
    1d array, float or int
        Contains heat of formation at T in kJ/kmol.
    """
    return np.array(sf_298) + np.array(a)*np.log(T/298.15) + np.array(b)*(T-298.15)\
        + np.array(c)/2*(T**2-298.15**2) + np.array(d)/3*(T**3-298.15**3)


def calc_gibbs_temp(T, hf_temp, sf_temp, multireaction=False):
    """
    Returns the gibbs energy of formation in kJ/kmol.
    Parameters
    ----------
    T : float or int
        Temperature in K.
    hf_temp : float, int, or 1d array
        Contains heat of formation at T in kJ/kmol.
    sf_temp : float, int, or 1d array
        Contains entropy of formation at T in kJ/kmol/K.
    Returns
    -------
    float, int, or 1d array
        Contains gibbs energy of reaction at T in kJ/kmol.
    """
    return np.array(hf_temp) - T*np.array(sf_temp)