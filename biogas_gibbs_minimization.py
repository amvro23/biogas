import numpy as np
from biogas_data_nist import (CH4, CO2, H2O, H2, CO, temp_range, C_,
                              a_jk, a_jk_inlet, inlet_species, atoms_coeffs, atoms_number,
                              HF298, A, B, C, D, E, F, G, H)
from biogas_thermodynamics_nist import std_hr, std_sr, std_gibbs
...
class GasEquilibrium:
    ...
    def conversions(self):
        yj = self.multiple_temperatures(temp_range)
    ...
    def plot_conversions_yields(self):
        ...
        ax.plot(temp_range - 273.15, self.conversions['Xch4'], label = 'CH$_4$')
        ax.plot(temp_range - 273.15, self.conversions['Xco2'], label = 'CO$_2$')
        ax.plot(temp_range - 273.15, self.yields['Yh2'], label = 'H$_2$')
        ax.plot(temp_range - 273.15, self.yields['Yco'], label = 'CO')
        ...
        ax.legend()
        ax_b = ax.twinx()
        ax_b.plot(temp_range-273.15, self.ratio_h2_co['H2/CO'], "k:", label = 'H$_2$/CO', lw = 1)
    ...
    def plot_molar_ratio(self):
        yj = self.multiple_temperatures(temp_range)
        fig, ax = plt.subplots(figsize = (6,4), dpi = 150)
        ax.plot(temp_range - 273.15, yj[CH4], label = 'CH$_4$')
        ax.plot(temp_range - 273.15, yj[CO2], label = 'CO$_2$')
        ax.plot(temp_range - 273.15, yj[H2O], label = 'H$_2$O')
        ax.plot(temp_range - 273.15, yj[H2], label = 'H$_2$')
        ax.plot(temp_range - 273.15, yj[CO], label = 'CO')
...
class CarbonEquilibrium(GasEquilibrium):
    ...
    def conversions_carbon(self):
        yj = self.multiple_temperatures_carbon(temp_range)
        Xch4 = ((inlet_species[CH4] - yj[CH4])/inlet_species[CH4])*100
        Xco2 = ((inlet_species[CO2] - yj[CO2])/inlet_species[CO2])*100    
        return {'Xch4': Xch4, 'Xco2': Xco2}
    ...
    def plot_conversions_yields_carbon(self):
        fig, ax = plt.subplots(figsize = (6,4), dpi = 150)
        ax.plot(temp_range - 273.15, self.conversions_carbon['Xch4'], label = 'CH$_4$')
        ax.plot(temp_range - 273.15, self.conversions_carbon['Xco2'], label = 'CO$_2$')
        ax.plot(temp_range - 273.15, self.yields_carbon['Yh2'], label = 'H$_2$')
        ax.plot(temp_range - 273.15, self.yields_carbon['Yco'], label = 'CO')
        ax.set_title('Equilibrium Conversions/Yields', fontsize=10, fontweight='bold')
        ax.set_ylabel('[%]', fontsize=12, fontweight='bold')
        ax.set_xlabel('Temperature $^o$C', fontsize=12, fontweight='bold')
        ax.grid(ls="-", alpha = 0.2)
        ax.legend()
        ax_b = ax.twinx()
        ax_b.plot(temp_range-273.15, self.ratio_h2_co_carbon['H2/CO'], "k:", label = 'H$_2$/CO', lw = 1)
        ax_b.legend(loc = "lower right")
        fig.show()
    ...
    def plot_molar_ratio_carbon(self):
        ...
        ax.plot(temp_range - 273.15, yj[H2], label = 'H$_2$')
        ax.plot(temp_range - 273.15, yj[CO], label = 'CO')
        ax.plot(temp_range - 273.15, yj[-1], label = 'C')
        ax.set_title('Equilibrium Compositions', fontsize=10, fontweight='bold')
        ax.set_ylabel('molar ratio', fontsize=12, fontweight='bold')
        ax.set_xlabel('Temperature $^o$C', fontsize=12, fontweight='bold')
        ax.grid(ls="-", alpha = 0.2)
